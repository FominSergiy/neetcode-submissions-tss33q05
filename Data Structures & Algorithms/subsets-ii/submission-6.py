class Solution:
    def subsetsWithDup(self, nums: List[int]) -> List[List[int]]:
        ...
        # take current number or skip dups and pass along
        # i tracks choices - once we get to last i, add to ans
        def backtrack(i: int, cur: list):
            if i == len(nums):
                ans.append(cur[:])
                return
            
            # choice 1 - use current number
            cur.append(nums[i])
            backtrack(i + 1, cur)
            cur.pop()

            # choice 2 - skip dups and use next num
